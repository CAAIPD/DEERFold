...
def load_models_from_command_line(args, config):
    ...
    # sd = get_fp32_state_dict_from_zero_checkpoint(args.checkpoint_path)
...
def main(args):
    ...
    if args.splabel.endswith('.csv'):
        sp = fill_org_dist(args.fasta, args.splabel)
        feature_dict['sp'] = sp
    else:
        print("Input distance info need to be in CSV file")
        sys.exit(0)
