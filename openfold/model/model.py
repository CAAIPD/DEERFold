...
class AlphaFold(nn.Module):
    ...
    def iteration(self, feats, m_1_prev, z_prev, x_prev, _recycle=True):
        ...
        if not self.config.sp_embedder.distograms:
            z = z + self.sp_embedder(feats["sp"])
        else:
            z = z + self.sp_embedder(feats["sp"] * 1000) # scale up to force signal
    ...
    def forward(self, batch):
        ...
        for cycle_no in range(num_iters):
            ...
            with torch.set_grad_enabled(is_grad_enabled and is_final_iter):
                ...
                # import numpy as np
                # tmp_out = self.aux_heads(outputs)
                # min_bin = 2.3125
                # max_bin = 21.6875
                # no_bins = 64
                # bin_centers = torch.linspace(min_bin, max_bin, no_bins)
                # logits = torch.nn.functional.softmax(tmp_out['distogram_logits'], dim=-1)
                # distance_map = torch.sum(logits * bin_centers.to(tmp_out['distogram_logits'].device), dim=-1)
                # distance_map_np = distance_map.cpu().numpy()
                # fig, ax = plt.subplots(figsize=(4, 4))
                # im = ax.imshow(distance_map_np, cmap='viridis', origin='lower', aspect='equal')
                # cbar = ax.figure.colorbar(im, ax=ax)
                # cbar.ax.set_ylabel('Distance (Å)', rotation=-90, va="bottom")
                # ax.set_title('Protein Distance Map')
                # ax.set_xlabel('Residue Index')
                # ax.set_ylabel('Residue Index')
                # plt.savefig('distance_map'+str(cycle_no)+'.png', dpi=300, bbox_inches='tight')
