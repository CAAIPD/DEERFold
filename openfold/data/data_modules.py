...
def process_csv(file_path, seq_length, num_dist=25):
    ...
    available_indices = list(unique_indices)
    if len(available_indices) < num_dist:
        num_dist = len(available_indices)

    selected_indices = random.sample(available_indices, num_dist)
    # print(selected_indices)
    selected_rows = [row for row in data if tuple(map(int, row[:2])) in selected_indices or tuple(map(int, row[:2][::-1])) in selected_indices]

    sp = np.zeros((seq_length, seq_length, 100))
    for row in selected_rows:
        i, j = map(int, row[:2])
        values = list(map(float, row[2:]))
        sp[i - 1][j - 1] = sp[j - 1][i - 1] = values
    ...
    def __init__(self,
        ...
        if(mapping_path is None):
            if(feature_dir is None):
                ...
                self.mapping = {
                    str(i):os.path.splitext(name)[0] 
                    for i, name in enumerate(os.listdir(feature_dir)) #if i < 10 # or mode == 'eval'
                }
        else:
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if(self.training_mode):        
            self.train_dataset = dataset_gen(
                ...
                feature_dir=self.train_feats_dir, #'/data/mchaourab/wut18/pdb_train/openfold_features'
                sp_dir=self.train_DEER_dir, #'/data/mchaourab/wut18/pdb_train/train2/sl/',
